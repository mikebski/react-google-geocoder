...
return {
    ...
    resetButton() {
        return (<button className={this.props.resetButtonClass} disabled={this.state.searching} type="button" onClick={(event) => {
        ...
        return (
            <div style={{width: "350px"}}>
                ...
                <form className={this.props.formClass}>
                    <fieldset>
                        {this.state.state === STATE.REQUEST_ERROR ? <p className={this.props.errorMessageClass}>{this.props.errorMessage}</p> : null}
                        <div className={this.props.inputLabelWrapperClass}>
                            <label className={this.props.labelClass} htmlFor={inputId}><span>{this.props.fieldLabel}</span></label>
                            <input className={this.props.inputClass} id={inputId} disabled={this.state.searching} value={this.state.address}
                                   onChange={(event) => this.changeAddress(event.target.value)}/>
                        </div>
                    </fieldset>
                    <div className={this.props.formButtonWrapperClass}>
                        <button className={this.props.searchButtonClass} disabled={this.state.searching || this.state.state === STATE.ONE_FOUND} type="button" onClick={(event) => {
                            component.searchAddress(component.state.address)
                        }}>{this.props.searchButtonLabel}
                        </button>
                        {this.resetButton()}
                        <button className={this.props.acceptButtonClass} disabled={this.state.state !== STATE.ONE_FOUND} type="button"
                        ...
                        <table className={this.props.tableClass + " popup"}>
                            ...
                            {component.state.foundAddresses.map((val, key) =>
                                <tr key={"id_" + key}>
                                    ...
                                    <td>
                                        <button className={this.props.selectButtonClass} type="button" onClick={(event) => component.setState({
